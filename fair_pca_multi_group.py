# Code taken from https://github.com/amazon-science/fair-pca
import numpy as np
import scipy
...
def standard_PCA(X, dim):
    ...
    Args:
    X (n x d numpy-array): training data; has points as rows
...
def apply_fair_PCA_to_dataset(train_dataset, target_dim, model_class, fairness_constraint,
                              tradeoff_param=0, standardize=True, fit_classifier=True,
    ...
    FOR THE METHOD TO WORK WELL IT IS REQUIRED THAT THE DATA IS (ROUGHLY) NORMALIZED (that is
    features have zero mean and unit variance); if standardize==True, the training dataset is
    normalized and the mean and variance of the training dataset is used to normalize test data
    ...
    Args:
        ...
        model_class (class): Python class for some model that exposes fit() and predict(), e.g.,
            LogisticRegression; should be a linear model for our method to work
        ...
        standardize (bool): whether to normalize the data or not; default is 'True'
        fit_classifier (bool): whether to fit a classifier for predicting labels
...
def apply_fair_PCA_to_dataset_multiple_groups(train_dataset, target_dim, model_class, fairness_constraint,
                              tradeoff_param=0, standardize=True, fit_classifier=True,
                              **model_hyperparameters):
    """
    applies a fair version of PCA to a dataset and then trains a classification model on the
    transformed dataset; returns a pipeline that first transforms given test data and then runs the
    classification model; this function allows for multiple demographic groups
    ...
    Args:
        train_dataset (tuple): tuple of (X_train, y_train, z_train), where X_train is a
            n x d numpy-array (with dtype==float or dtype==int) containing datapoints as rows,
            y_train is a 1-dim numpy array of length n with values in {0,1} encoding
            ground-truth labels, and z_train is a 1-dim numpy array of length n with values 
            in {0, ..., number of groups - 1} encoding protected attributes.
        target_dim (int): target dimension; must not be larger than d - number of groups + 1
        model_class (class): Python class for some model that exposes fit() and predict(), e.g.,
            LogisticRegression; should be a linear model for our method to work
        fairness_constraint (str): string specifying which constraint to apply; can be
            'selection_rate_parity' or 'true_positive_rate_parity'
    ...
    # checks inputs
    check_inputs(target_dim, fairness_constraint, tradeoff_param, x_train, y_train,
                 prot_attribute_train, multiple_groups=True)
    ...
    fair_PCA_method = FairPCA_multiple_groups(target_dim, standardize, tradeoff_param)
...
def apply_fair_PCA_equalize_covariance_to_dataset(train_dataset, target_dim, model_class,
                                                  fairness_constraint,
                                                  tradeoff_param=0, nr_evecs_cov_constraint=10,
                                                  standardize=True, fit_classifier=True,
                                                  **model_hyperparameters):
    """
    applies a fair version of PCA to a dataset and then trains a classification model on the
    transformed dataset; returns a pipeline that first transforms given test data and then runs the
    classification model --- compared to apply_fair_PCA_to_dataset / FairPCA,
    apply_fair_PCA_equalize_covariance_to_dataset / FairPCAEqualizeCovariance also aim to equalize
    the group-conditional covariance matrices of the projected data
    ...
    Args:
        ...
        tradeoff_param (float in [0, 1]): parameter trading off fairness vs accuracy; the smaller,
            the more fair is the representation
        nr_evecs_cov_constraint (int): number of eigenvectors to use for approximately
    ...
    if not isinstance(nr_evecs_cov_constraint, int):
        raise ValueError("The parameter 'nr_evecs_cov_constraint' needs to be an "
                         "integer")

    if not (nr_evecs_cov_constraint > target_dim and nr_evecs_cov_constraint < (
            x_train.shape[1] - 1)):
        warnings.warn(
            "The parameter 'nr_evecs_cov_constraint' should be larger than the target dimension "
...
def apply_fair_kernel_PCA_to_dataset(train_dataset, target_dim, model_class, fairness_constraint,
                                     tradeoff_param=0,
                                     kernel_function="rbf", degree_kernel=3, gamma_kernel="scale",
                                     standardize=True, fit_classifier=True,
                                     **model_hyperparameters):
    """
    ...
    Args:
        ...
        kernel_function (str): specifies the kernel function to be used; has to be one of {‘poly’,
            ‘rbf’, ‘sigmoid’}; default is 'rbf'
        degree_kernel (int): degree of the polynomial kernel function ('poly'); default is 3
        gamma_kernel (float or 'scale' or 'auto'): kernel coefficient for ‘rbf’, ‘poly’ and
            ...
            default is 'scale'
        standardize (bool): whether to normalize the data or not (normalization is done implicitly
            in the feature space); default is 'True'
        fit_classifier (bool): whether to fit a classifier for predicting labels
        **model_hyperparameters: hyperparameters that need to be fed to the model_class constructor
...
class MyPipeline(Pipeline):
    """
    Code taken from https://stackoverflow.com/questions/33469633/how-to-transform-items-using-
    sklearn-pipeline/33471658
    """
...
class FairPCA:
    ...
    def fit(self, X, prot_attribute):
        ...
        # compute H using matmul or multi_dot --- not clear which one is faster
        # H = np.matmul(np.matmul(np.transpose(R),np.matmul(np.transpose(X),X)),R)
        H = np.linalg.multi_dot([np.transpose(R), np.transpose(X), X, R])

        # compute eigenvectors using numpy.linalg.eigh or scipy.linalg.eigh --- the latter should be
        # faster when dimension is rather small
        # using numpy
...
class FairPCA_multiple_groups:
    """
    implements a fair version of PCA dimensionality reduction for multiple groups
    """
    ...
    def fit(self, X, prot_attribute):
        """
        X(n x d numpy - array): data matrix containing points as rows
        prot_attribute (1-dim numpy array of length n with values in {0, 1, ..., number of groups - 1}): i-th entry is
            protected attribute of i-th point
        """
        nr_of_groups = np.amax(prot_attribute) + 1
        
        if not all([np.array_equal(np.unique(prot_attribute), np.arange(nr_of_groups)),
                    len(prot_attribute.shape) == 1,
                    X.shape[0] == len(prot_attribute)]):
            raise ValueError("Array with protected attributes must be a 1-dim numpy array of the "
                             "same length as there are points in X and with values in {0, 1, ..., number of groups - 1}")
        ...
        Z = np.zeros((X.shape[0], nr_of_groups))
        Z[range(X.shape[0]), prot_attribute] = 1
        Z -= np.tile(np.mean(Z,axis=0),(X.shape[0],1))
        R = scipy.linalg.null_space(np.matmul(np.transpose(Z), X))
...
class FairPCAEqualizeCovariance:
    ...
    def __init__(self, target_dim, standardize, tradeoff_param, nr_evecs_cov_constraint):
        """
        target_dim (int): target dimension
        standardize (bool): whether to normalize the data or not; default is 'True'
        tradeoff_param (float in [0, 1]): parameter trading off fairness vs accuracy; the smaller,
        ...
        self.tradeoff_param = tradeoff_param
        self.transformation_matrix_standard_PCA = np.nan
        self.nr_eigenvectors_cov_constraint = nr_evecs_cov_constraint
    ...
    def fit(self, X, prot_attribute):
        ...
        eig2, eigenvectors2 = solve_standard_eigenproblem_for_largest_eigenvalues(H, self.dimension)
...
def construct_pipeline(x_train, y_train, prot_attribute_train, fairness_constraint, fair_PCA_method,
    ...
    """
    constructs MyPipeline object comprising fair PCA transformation followed by a trained classifier
    """
...
def check_inputs(target_dim, fairness_constraint, tradeoff_param, x_train, y_train,
                 prot_attribute_train, multiple_groups = False):
    """
    ...
    if multiple_groups == False:
        ...
        nr_of_groups = np.amax(prot_attribute_train)+1
        if not all([isinstance(target_dim, int), target_dim > 0,
                    target_dim <= (x_train.shape[1] - nr_of_groups + 1)]):
            raise ValueError("Target dimension needs to be an integer greater than zero and not "
                                "greater than the dimension of the data - nr_of_groups + 1 ")
    ...
    if not (np.array_equal(np.unique(y_train), [0, 1]) and len(y_train.shape) == 1):
        raise ValueError("Array with ground-truth labels must be a 1-dim numpy array with "
                         "values in {0,1}")
    ...
    if multiple_groups == False:
        if not (np.array_equal(np.unique(prot_attribute_train), [0, 1]) and
         ...
         if not (np.array_equal(np.unique(prot_attribute_train), np.arange(np.amax(prot_attribute_train)+1)) and
                len(prot_attribute_train.shape) == 1):
            raise ValueError("Array with potected attributes must be a 1-dim numpy array with "
                            "values in {0,1,...,number of groups-1}")
